...
CREATE TABLE Warehouses (
    ...
    address TEXT NOT NULL, -- Detailed address of the warehouse
...
CREATE Table WarehouseStorage (
    ...
    order_type VARCHAR(20) NOT NULL CHECK (order_type IN ('Order In', 'Order Out')), -- Type of order
...
CREATE TABLE TripOrders (
    ...
    status VARCHAR(50) NOT NULL DEFAULT 'Pending Assignment' CHECK (status IN ('Pending Assignment', 'Assigned', 'In Progress', 'Completed', 'Cancelled')), -- Status of the trip itself
    ...
    is_completed BOOLEAN DEFAULT FALSE -- Whether the trip has been completed
...
CREATE TABLE VehicleStatusHistory (
    ...
    status_timestamp TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP, -- When this status/location was recorded
    status VARCHAR(50) NOT NULL CHECK (status IN ('Offline', 'Idle', 'Assigned', 'En Route to Pickup', 'Loading', 'Delivering', 'Unloading', 'Returning', 'Maintenance')), -- Vehicle status at this time
...
COMMENT ON COLUMN VehicleStatusHistory.status_timestamp IS 'Timestamp when the recorded status/location became effective.';
COMMENT ON COLUMN VehicleStatusHistory.status IS 'Detailed vehicle status, e.g., Offline, Idle, Assigned, En Route to Pickup, Loading, Delivering, Unloading, Returning, Maintenance.';
...
CREATE TABLE OrderBatches (
    ...
    status VARCHAR(50) NOT NULL DEFAULT 'Open' CHECK (status IN ('Open', 'Ready for Pickup', 'In Transit', 'Completed', 'Cancelled')), -- Status of the batch itself
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    ...
    is_completed BOOLEAN DEFAULT FALSE -- Whether the batch has been completed
);
...
COMMENT ON COLUMN OrderBatches.status IS 'Status of the batch itself: Open (adding orders), Ready for Pickup, In Transit, Completed, Cancelled.';
...
CREATE TABLE Orders (
    ...
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    estimated_arrival_at_destination TIMESTAMP WITH TIME ZONE, -- ETA at the destination address
    end_time TIMESTAMP WITH TIME ZONE, -- Time when the order was completed
    is_completed BOOLEAN DEFAULT FALSE -- Whether the order has been completed
);
...
CREATE TABLE OrderStatusHistory (
    history_id SERIAL PRIMARY KEY,
    order_id INT NOT NULL REFERENCES Orders(order_id) ON DELETE CASCADE ON UPDATE CASCADE,
    status_timestamp TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT CURRENT_TIMESTAMP, -- When this status became effective
    status VARCHAR(50) NOT NULL, -- Order status, e.g., Created, ReceivedAtWarehouse, Batched, AssignedToTrip, OutForDelivery, Delivered, AttemptFailed, Cancelled
...
COMMENT ON COLUMN OrderStatusHistory.status_timestamp IS 'Timestamp when the recorded status became effective.';
COMMENT ON COLUMN OrderStatusHistory.status IS 'E.g., Created, ReceivedAtWarehouse, Batched, AssignedToTrip, OutForDelivery, Delivered, AttemptFailed, Cancelled';
...
CREATE INDEX idx_vehicle_status_history_latest ON VehicleStatusHistory (vehicle_id, status_timestamp DESC);
...
CREATE INDEX idx_triporders_status ON TripOrders(status);

-- OrderBatches
CREATE INDEX idx_orderbatches_batch_code ON OrderBatches(batch_code);
CREATE INDEX idx_orderbatches_status ON OrderBatches(status);
...
-- OrderStatusHistory
CREATE INDEX idx_order_status_history_latest ON OrderStatusHistory (order_id, status_timestamp DESC);
...
CREATE INDEX idx_batchorderitems_order_id ON BatchOrderItems(order_id); -- For lookups by order
